...
from pur.core.misc.expiring_set import ExpiringSet
from pur.core.notification.Observable import Observable
from pur.core.notification.ObservableEvent import ObservableEvent
...
class P2PPeerManager(P2PBaseObserver):
    ...
    def trusted_peer(self, channel: P2PProtocol):
        ...
        if channel.valid_message_count < config.dev.trust_min_mspurount:
            return False
    ...
    def new_channel(self, channel):
        ...
        self._peer_node_status[channel] = pur_pb2.NodeChainState(block_number=0,
                                                                 header_hash=b'',
                                                                 cumulative_difficulty=b'\x00' * 32,
                                                                 timestamp=ntp.getTime())
        channel.register(purlegacy_pb2.LegacyMessage.VE, self.handle_version)
        channel.register(purlegacy_pb2.LegacyMessage.PL, self.handle_peer_list)
        channel.register(purlegacy_pb2.LegacyMessage.CHAINSTATE, self.handle_chain_state)
        channel.register(purlegacy_pb2.LegacyMessage.SYNC, self.handle_sync)
        channel.register(purlegacy_pb2.LegacyMessage.P2P_ACK, self.handle_p2p_acknowledgement)
    ...
    def handle_version(self, source, message: purlegacy_pb2.LegacyMessage):
        """
        Version
        ...
        """
        self._validate_message(message, purlegacy_pb2.LegacyMessage.VE)

        if not message.veData.version:
            msg = purlegacy_pb2.LegacyMessage(
                func_name=purlegacy_pb2.LegacyMessage.VE,
                veData=purlegacy_pb2.VEData(version=config.dev.version,
    ...
    def handle_peer_list(self, source, message: purlegacy_pb2.LegacyMessage):
        P2PBaseObserver._validate_message(message, purlegacy_pb2.LegacyMessage.PL)
    ...
    def handle_sync(self, source, message: purlegacy_pb2.LegacyMessage):
        P2PBaseObserver._validate_message(message, purlegacy_pb2.LegacyMessage.SYNC)
        if message.syncData.state == '':
            if source.factory.synced:
                source.send_sync(synced=True)

    @staticmethod
    def send_node_chain_state(dest_channel, node_chain_state: pur_pb2.NodeChainState):
        # FIpurE: Not sure this belongs to peer management
        msg = purlegacy_pb2.LegacyMessage(func_name=purlegacy_pb2.LegacyMessage.CHAINSTATE,
    ...
    def broadcast_chain_state(self, node_chain_state: pur_pb2.NodeChainState):
        # FIpurE: Not sure this belongs to peer management
    ...
    def handle_chain_state(self, source, message: purlegacy_pb2.LegacyMessage):
        P2PBaseObserver._validate_message(message, purlegacy_pb2.LegacyMessage.CHAINSTATE)
    ...
    def handle_p2p_acknowledgement(self, source, message: purlegacy_pb2.LegacyMessage):
        P2PBaseObserver._validate_message(message, purlegacy_pb2.LegacyMessage.P2P_ACK)
    ...
    def get_peers_stat(self) -> list:
        ...
        for source in list(self.peer_node_status.keys()):
            try:
                peer_stat = pur_pb2.PeerStat(peer_ip=source.peer.ip.encode(),
