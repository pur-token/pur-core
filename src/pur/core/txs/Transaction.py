...
from pypurlib.pypurlib import bin2hstr, purHelper, purssFast
...
from pur.core.txs import build_tx as main_build_tx
...
class Transaction(object, metaclass=ABCMeta):
    ...
    def __init__(self, protobuf_transaction=None):
        ...
        if protobuf_transaction is None:
            self._data = pur_pb2.Transaction()
    ...
    def addr_from(self):
        ...
        return bytes(purHelper.getAddress(self.PK))
    ...
    @staticmethod
    def from_pbdata(pbdata: pur_pb2.Transaction):
    ...
    @staticmethod
    def get_slave(tx):
        addr_from_pk = bytes(purHelper.getAddress(tx.PK))
    ...
    def set_affected_address(self, addresses_set: set):
        ...
        addresses_set.add(bytes(purHelper.getAddress(self.PK)))
    ...
    def validate_all(self, state_container: StateContainer, check_nonce=True) -> bool:
        ...
        addr_from_pk = bytes(purHelper.getAddress(self.PK))
    ...
    def _coinbase_filter(self):
        if config.dev.coinbase_address in [bytes(purHelper.getAddress(self.PK)), self.master_addr]:
    ...
    def validate_or_raise(self, verify_signature=True) -> bool:
        ...
        if verify_signature:
            ...
            # TODO: Review Juan
            # if not purSS.validate_signature(self.signature, self.PK):
            #     raise ValueError("Invalid purss signature")

            if not purssFast.verify(self.get_data_hash(),
                                   self.signature,
                                   self.PK):
                raise ValueError("Invalid purss signature")
    ...
    def get_message_hash(self):
        # FIpurE: refactor, review that things are not recalculated too often, cache, etc.
        return self.txhash

    def to_json(self):
        # FIpurE: Remove once we move completely to protobuf
