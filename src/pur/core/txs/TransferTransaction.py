...
class TransferTransaction(Transaction):
    """
    SimpleTransaction for the transaction of pur from one wallet to another.
    """
    ...
    @staticmethod
    def create(addrs_to: list, amounts: list, message_data: Union[bytes, None], fee: int, purss_pk, master_addr: bytes = None):
        transaction = TransferTransaction()

        if master_addr:
            transaction._data.master_addr = master_addr

        transaction._data.public_key = bytes(purss_pk)
        ...
        transaction._data.fee = int(fee)  # FIpurE: Review conversions for quantities
