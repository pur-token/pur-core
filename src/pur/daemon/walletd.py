...
from pypurlib.pypurlib import hstr2bin, mnemonic2bin, bin2hstr, purHelper

from pur.core import config
from pur.core.AddressState import AddressState
from pur.daemon.helper import logger
from pur.daemon.helper.DaemonHelper import WalletDecryptionError, Wallet, UNRESERVED_OTS_INDEX_START
from pur.services.WalletAPIService import WalletAPIService
from pur.generated import pur_pb2, pur_pb2_grpc, purwallet_pb2
from pur.generated.purwallet_pb2_grpc import add_WalletAPIServicer_to_server
...
pid = os.path.join(config.user.wallet_dir, 'pur_walletd.pid')


class WalletD:
    def __init__(self):
        self._wallet_path = os.path.join(config.user.wallet_dir, 'walletd.json')
        self._public_stub = pur_pb2_grpc.PublicAPIStub(grpc.insecure_channel(config.user.public_api_server))
        self._wallet = None
        self._passphrase = None
        self.load_wallet()

    def to_plain_blocks(self, block):
        pheader = purwallet_pb2.PlainBlockHeader()
        ...
        pblock = purwallet_pb2.PlainBlock()
        ...
        for genesis_balance in block.genesis_balance:
            pgb = purwallet_pb2.PlainGenesisBalance()
    ...
    def _get_wallet_index_purss(self, signer_address: str, ots_index: int):
        index, _ = self._wallet.get_address_item(signer_address)
        if index is None:
            raise Exception("Signer Address Not Found ", signer_address)
        purss = self._wallet.get_purss_by_index(index, self._passphrase)
        if ots_index > 0:
            purss.set_ots_index(ots_index)
        return index, purss

    def get_pk_list_from_purss_list(self, slave_purss_list):
        return [purss.pk for purss in slave_purss_list]
    ...
    def add_new_address_with_slaves(self,
        ...
        purss = self._wallet.add_new_address(height, hash_function, True)
        slave_purss_list = self._wallet.add_slave(index=-1,
                                                 height=height,
        ...
        slave_pk_list = self.get_pk_list_from_purss_list(slave_purss_list)
        slave_tx = self.generate_slave_tx(purss.pk, slave_pk_list)
        self.sign_and_push_transaction(slave_tx, purss, -1)
    ...
    def add_address_from_seed(self, seed=None) -> str:
        ...
        address_from_seed = purSS.from_extended_seed(bin_seed)
        if self._wallet.get_purss_by_qaddress(address_from_seed.qaddress, self._passphrase):
            raise Exception("Address is already in the wallet")
        self._wallet.append_purss(address_from_seed)
        self._encrypt_last_item()
        self._wallet.save()
    ...
    def get_recovery_seeds(self, qaddress: str):
        ...
        purss = self._wallet.get_purss_by_qaddress(qaddress, self._passphrase)
        if purss:
            logger.info("Recovery seeds requested for %s", qaddress)
            return purss.hexseed, purss.mnemonic
    ...
    def get_address_state(self, qaddress: str) -> AddressState:
        request = pur_pb2.GetAddressStateReq(address=bytes(hstr2bin(qaddress[1:])))
    ...
    def sign_and_push_transaction(self,
                                  ...
                                  slave_index=None,
                                  enable_save=True):
        logger.info("Signing %s transaction by %s | OTS index %s", tx.type, purss.qaddress, purss.ots_index)
        tx.sign(purss)
        ...
        if enable_save:
            if slave_index is None:  # noqa
                self._wallet.set_ots_index(index, purss.ots_index)  # Move to next OTS index before broadcasting txn
            else:
                self._wallet.set_slave_ots_index(index, group_index, slave_index, purss.ots_index)
        ...
        if push_transaction_resp.error_code != pur_pb2.PushTransactionResp.SUBMITTED:
            raise Exception(push_transaction_resp.error_description)

    def try_txn_with_last_slave(self, item, index, group_index, purss=None):
        ...
        if not purss:
            ...
            purss = self._wallet.get_purss_by_item(target_address_item, ots_index)
        else:
            purss.set_ots_index(ots_index)
    ...
    def is_slave(self, master_address: bytes, slave_pk: bytes) -> bool:
        request = pur_pb2.IsSlaveReq(master_address=master_address,
                                     slave_pk=slave_pk)
    ...
    def get_unused_ots_index(self, address: bytes, unused_ots_index_from: int) -> Optional[int]:
        request = pur_pb2.GetOTSReq(address=address,
    ...
    def get_slave(self, master_qaddress):
        ...
        if not address_state.validate_slave_with_access_type(str(bytes(hstr2bin(slave.pk))), [0]):
            ...
            tx = self.generate_slave_tx(purss.pk,
                                        slaves_pk,
            ...
            if len(item.slaves) > 1:
                if self.try_txn_with_last_slave(item, index, group_index, purss):
                    return index, len(item.slaves) - 2, len(item.slaves[group_index]) - 1, purss

        else:
            if len(item.slaves) > 1:
                group_index = len(item.slaves) - 2
                purss = self.try_txn_with_last_slave(item, index, group_index)
                if purss:
                    return index, group_index, len(item.slaves[group_index]) - 1, purss
            ...
            for slave_index, slave in enumerate(last_slaves):
                ...
                if slave_index + 1 == len(last_slaves) and slave.index > 2 ** slave.height - 100:
                    ...
                    curr_slave_purss = self._wallet.get_purss_by_item(slave, ots_index)

                    slave_purss_list = self._wallet.add_slave(index=index,
                                                             height=slave.height,
                                                             number_of_slaves=config.user.number_of_slaves,
                ...
                slave_purss = self._wallet.get_purss_by_item(slave, ots_index)

                return index, group_index, slave_index, slave_purss
    ...
    def get_slave_purss(self, master_qaddress):
        index, group_index, slave_index, slave_purss = self.get_slave(master_qaddress)
    ...
    def verify_ots(self, signer_address, purss, user_ots_index):
        addr_state = self.get_address_state(signer_address)

        verified_ots_index = self.get_unused_ots_index(addr_state.address, purss.ots_index)

        if verified_ots_index is None:  # noqa
            raise Exception("No Unused OTS key found")

        if user_ots_index > 0:
            if verified_ots_index != purss.ots_index:
                raise Exception("Used OTS Index %s", user_ots_index)
        else:
            purss.set_ots_index(verified_ots_index)
    ...
    def relay_transfer_txn(self,
                           ...
                           ots_index: int):
        self.authenticate()
        index, purss = self._get_wallet_index_purss(signer_address, ots_index)
        self.verify_ots(signer_address, purss, user_ots_index=ots_index)
        ...
        self.sign_and_push_transaction(tx, purss, index)
    ...
    def relay_transfer_txn_by_slave(self,
                                    ...
                                    master_qaddress):
        self.authenticate()
        index, group_index, slave_index, slave_purss = self.get_slave_purss(master_qaddress)
        if slave_index == -1:
        ...
        tx = TransferTransaction.create(addrs_to=self.qaddresses_to_address(qaddresses_to),
                                        ...
                                        message_data=None,
                                        fee=fee,
                                        purss_pk=slave_purss.pk,
                                        master_addr=self.qaddress_to_address(master_qaddress))

        self.sign_and_push_transaction(tx, slave_purss, index, group_index, slave_index)
    ...
    def relay_token_txn(self,
        ...
        for idx, qaddress in enumerate(qaddresses):
            initial_balances.append(pur_pb2.AddressAmount(address=self.qaddress_to_address(qaddress),
                                                          amount=amounts[idx]))
    ...
    def get_mini_transactions_by_address(self, qaddress: str, item_per_page: int, page_number: int) -> tuple:
        ...
        response = self._public_stub.GetMiniTransactionsByAddress(
            pur_pb2.GetMiniTransactionsByAddressReq(address=address,
    ...
    def get_transaction(self, tx_hash: str):
        ...
        response = self._public_stub.GetTransaction(pur_pb2.GetTransactionReq(tx_hash=txhash))
    ...
    def get_balance(self, qaddress: str) -> int:
        address = self.qaddress_to_address(qaddress)
        response = self._public_stub.GetBalance(pur_pb2.GetBalanceReq(address=address))
    ...
    def get_total_balance(self) -> int:
        ...
        response = self._public_stub.GetTotalBalance(pur_pb2.GetTotalBalanceReq(addresses=addresses))
        return response.balance

    def get_ots(self, qaddress: str):
        address = self.qaddress_to_address(qaddress)
        response = self._public_stub.GetOTS(pur_pb2.GetOTSReq(address=address))
        return response.ots_bitfield_by_page, response.next_unused_ots_index, response.unused_ots_index_found

    def get_height(self) -> int:
        response = self._public_stub.GetHeight(pur_pb2.GetHeightReq())
        return response.height

    def get_block(self, header_hash: str):
        headerhash = bytes(hstr2bin(header_hash))
        response = self._public_stub.GetBlock(pur_pb2.GetBlockReq(header_hash=headerhash))
        return self.to_plain_blocks(response.block)

    def get_block_by_number(self, block_number: int):
        response = self._public_stub.GetBlockByNumber(pur_pb2.GetBlockByNumberReq(block_number=block_number))
        return self.to_plain_blocks(response.block)

    def get_address_from_pk(self, pk: str) -> str:
        return self.address_to_qaddress(purHelper.getAddress(bytes(hstr2bin(pk))))

    def get_node_info(self):
        return self._public_stub.GetNodeState(pur_pb2.GetNodeStateReq())
...
def main():
    ...
    daemon = Daemonize(app="pur_walletd", pid=pid, action=run)
    daemon.start()
