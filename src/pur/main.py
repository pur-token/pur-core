...
from pur.core.misc import ntp, logger, set_logger
from pur.core.purnode import purNode
from pur.services.services import start_services
...
def parse_arguments():
    ...
    parser.add_argument('--miningAddress', dest='mining_address', required=False,
                        help="pur Wallet address on which mining reward has to be credited.")
...
def main():
    ...
    logger.debug("=====================================================================================")
    logger.info("pur Path: %s", args.pur_dir)
    ...
    purnode = purNode(mining_address=mining_address)
    purnode.set_chain_manager(chain_manager)

    set_logger.set_logger(args, purnode.sync_state)

    #######
    # NOTE: Keep assigned to a variable or might get collected
    admin_service, grpc_service, mining_service, debug_service = start_services(purnode)

    purnode.start_listening()

    purnode.start_pow(args.mining_thread_count)

    logger.info('pur blockchain ledger %s', config.dev.version)
    if config.user.mining_enabled:
