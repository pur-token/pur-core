...
def create_tx(addrs_to, amounts, signing_purss, nonce):
    tx = TransferTransaction.create(addrs_to=addrs_to,
                                    amounts=amounts,
                                    message_data=None,
                                    fee=0,
                                    purss_pk=signing_purss.pk)
    tx.sign(signing_purss)
    tx._data.nonce = nonce
    return tx


def get_migration_transactions(signing_purss, filename):
    transactions = []
    ...
    for addr in json_data:
        ...
        if count % output_limit == 0:
            transactions.append(create_tx(addrs_to, amounts, signing_purss, count // output_limit))

            addrs_to = []
            amounts = []

    if addrs_to:
        transactions.append(create_tx(addrs_to, amounts, signing_purss, (count // output_limit) + 1))
...
def main():
    ...
    dist_purss = purSS.from_extended_seed(seed)

    transactions = get_migration_transactions(signing_purss=dist_purss, filename=filename)
    ...
    block = Block.create(dev_config=config.dev,
                         ...
                         miner_address=dist_purss.address,
                         seed_height=None,
                         seed_hash=None)

    block.set_nonces(config.dev, 0, 0)

    block._data.genesis_balance.extend([pur_pb2.GenesisBalance(address=config.dev.coinbase_address,
