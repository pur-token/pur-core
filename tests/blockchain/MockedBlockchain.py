...
class MockedBlockchain(object):
    ...
    def __init__(self, purnode, time_mock, ntp_mock):
        ...
        self.alice_purss = get_alice_purss(purss_height=required_height)
        self.bob_purss = get_bob_purss()

    def create_block(self, prev_hash, mining_address=None):
        if not mining_address:
            mining_address = self.alice_purss.address
        transactions = []
        block_prev = self.purnode.get_block_from_hash(prev_hash)
        block_idx = block_prev.block_number + 1

        if block_idx == 1:
            slave_tx = SlaveTransaction.create(slave_pks=[self.bob_purss.pk],
                                               access_types=[0],
                                               fee=0,
                                               purss_pk=self.alice_purss.pk)
            slave_tx.sign(self.alice_purss)
        ...
        dev_config = self.purnode._chain_manager.get_config_by_block_number(block_new.block_number)
        while not self.purnode._chain_manager.validate_mining_nonce(blockheader=block_new.blockheader,
    ...
    def validate(self, block):
        if not block.validate(self.purnode._chain_manager, {}):
    ...
    def add_block(self, block):
        self.validate(block)
        return self.purnode._chain_manager.add_block(block)

    def add_new_block(self, mining_address=None):
        block_prev = self.purnode.get_block_last()
    ...
    def create(num_blocks, mining_address=None):
        ...
        with mock.patch('pur.core.misc.ntp.getTime') as ntp_mock, \
                set_pur_dir('no_data'), \
                State() as state, \
            ...
            try:
                ...
                mock_blockchain = MockedBlockchain(purnode, time_mock, ntp_mock)
