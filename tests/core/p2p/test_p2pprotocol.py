...
from pur.core.p2p import p2pPeerManager
from pur.core.p2p.p2pfactory import P2PFactory, p2p_msg_priority
...
# FIpurE: These tests will soon be removed
class TestP2PProtocol(TestCase):
    def __init__(self, *args, **kwargs):
        super(TestP2PProtocol, self).__init__(*args, **kwargs)

    def setUp(self):
        with patch('pur.core.misc.ntp.getTime', new=replacement_getTime):
        ...
        self.channel.factory._pur_node = Mock(autospec=purNode)
        self.channel.factory._pur_node.peer_manager = Mock(autospec=p2pPeerManager)
        self.channel.factory._pur_node.peer_manager.is_banned = MagicMock(return_value=False)
        self.channel.factory._pur_node.peer_manager.trusted_addresses = MagicMock(return_value=[])
    ...
    @patch('pur.core.misc.ntp.getTime')
    @patch('pur.core.p2p.p2pprotocol.logger', autospec=True)
    @patch('pur.core.p2p.p2pprotocol.config.dev', autospec=True)
    def test_dataReceived_too_big(self, config_dev, logger, getTime):
        ...
        config_dev.trust_min_mspurount = 10
        getTime.return_value = 1525078652.9991353
