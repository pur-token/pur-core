...
@patch('pur.core.processors.TxnProcessor.logger')
@patch('pur.core.txs.Transaction.Transaction.get_slave')
class TestTxnProcessor(TestCase):
    def setUp(self):
        ...
        tx_attrs = {
            ...
            'validate_extended.return_value': True,  # Master/slave purSS tree validation; balance & fee, OTS key reuse
    ...
    def test_txnprocessor_all_valid(self, m_get_slave, m_logger):
        # Transaction.get_slave() gives you the slave's Qaddress, if the TXN is signed by a slave purSS tree.
        # If it's a normal TXN signed by the master purSS tree, it returns None. Since we mocked out validate_extended(),
