...
from tests.misc.helper import get_alice_purss, get_slave_purss, get_random_purss, set_pur_dir
...
class TestAddressState(TestCase):
    ...
    @patch('pur.core.AddressState.AddressState.transaction_hashes', new_callable=PropertyMock)
    ...
    def test_ots_key_validation(self):
        random_purss = get_random_purss(purss_height=4)
        addr = AddressState.get_default(random_purss.address)
        ots_indexes = list(range(0, 2 ** random_purss.height))
        shuffle(ots_indexes)
    ...
    def test_get_unused_ots_index(self):
        random_purss = get_random_purss(purss_height=4)
        addr_state = AddressState.get_default(random_purss.address)
    ...
    def test_get_unused_ots_index2(self):
        ...
        try:
            random_purss = get_random_purss(purss_height=8)
            addr_state = AddressState.get_default(random_purss.address)
            self.assertEqual(addr_state.get_unused_ots_index(), 0)
            addr_state.set_ots_key(0)
            self.assertEqual(addr_state.get_unused_ots_index(), 1)
    ...
    def test_put_addresses_state(self):
        with set_pur_dir('no_data'):
            with State() as state:
                ...
                alice_state2 = OptimizedAddressState.get_optimized_address_state(state, alice_purss.address)
