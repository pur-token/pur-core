...
from tests.misc.helper import get_alice_purss, get_bob_purss, get_slave_purss
...
from pur.crypto.misc import merkle_tx_hash

from tests.misc.helper import replacement_getTime, set_pur_dir
...
class TestBlock(TestCase):
    ...
    def test_validate(self):
        ...
        with patch('pur.core.Block.Block.transactions', new_callable=PropertyMock) as m_transactions:
            m_transactions.return_value = []
            result = self.block.validate(m_chain_manager, OrderedDict())
            self.assertFalse(result)

        # There was a problem with the CoinBase TX
        with patch('pur.core.txs.CoinBase.CoinBase._validate_extended') as m_validate_extended:
...
@patch('pur.core.misc.ntp.getTime', new=replacement_getTime)
@patch('pur.core.txs.CoinBase.CoinBase.apply', name='mock CoinBase.apply()',
       return_value=True)
@patch('pur.core.txs.TransferTransaction.TransferTransaction.apply',
       name='mock TransferTransaction.apply()', return_value=True)
@patch('pur.core.txs.Transaction.Transaction.validate_slave',
       name='mock Transaction.validate_slave()', return_value=True)
@patch('pur.core.txs.TransferTransaction.TransferTransaction.validate', name='mock Transfer.validate()',
       return_value=True)
class TestBlockApplyStateChanges(TestCase):
    ...
    def setUp(self):
        ...
        self.tx1 = TransferTransaction.create(addrs_to=[self.bob.address], amounts=[10],
                                              message_data=None, fee=1, purss_pk=self.alice.pk)
        self.tx2 = TransferTransaction.create(addrs_to=[self.bob.address], amounts=[10],
                                              message_data=None, fee=1, purss_pk=self.slave.pk,
    ...
    def test_bad_nonce_or_ots_reused(self,
                                     ...
                                     m_TransferTransaction_apply_state_changes,
                                     m_CoinBase_apply_state_changes):
        # If a TX was signed by a Slave purSS, apply_state_changes() should check against the Slave's AddressState.nonce.
    ...
    def test_last_block(self,
        ...
        with patch("pur.core.Block.Block.get_block_by_number") as mock_get_block_by_number:
