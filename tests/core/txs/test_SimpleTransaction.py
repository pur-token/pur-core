...
from tests.misc.helper import get_alice_purss, get_bob_purss, get_slave_purss, replacement_getTime, set_pur_dir
...
class TestSimpleTransaction(TestCase):
    ...
    def test_validate_tx(self, m_logger):
        # If we change amount, fee, addr_from, addr_to, (maybe include purss stuff) txhash should change.
    ...
    @patch('pur.core.txs.Transaction.config')
    def test_validate_tx_invalid(self, m_config, m_logger):
        ...
        with patch('pur.core.txs.TransferTransaction.TransferTransaction.amounts',
        ...
        with patch('pur.core.txs.TransferTransaction.TransferTransaction.fee', new_callable=PropertyMock) as m_fee:
            m_fee.return_value = -1
            with self.assertRaises(ValueError):
                self.tx.validate_or_raise()
        with patch('pur.core.txs.TransferTransaction.TransferTransaction.addrs_to',
        ...
        with patch('pur.core.txs.TransferTransaction.TransferTransaction.addrs_to',
            ...
            with self.assertRaises(ValueError):
                self.tx.validate_or_raise()
        with patch('pur.core.txs.TransferTransaction.TransferTransaction.addr_from',
    ...
    def test_apply_state_changes_for_PK(self, m_logger):
        ...
        Curiously enough, if the TX was signed by a master purSS tree, it doesn't add this tx's txhash to the list of
    ...
    def test_apply_state_changes_for_PK_master_slave_purSS(self, m_logger):
        """
        If the TX was signed by a slave purSS, the slave purSS's AddressState should be updated (not the master's).
        :return:
        """
        tx = TransferTransaction.create(
            addrs_to=[self.bob.address],
    ...
    def test_revert_state_changes_for_PK_master_slave_purSS(self, m_logger):
        addresses_state = {
            self.alice.address: OptimizedAddressState.get_default(self.alice.address),
            self.slave.address: OptimizedAddressState.get_default(self.slave.address)
        }
