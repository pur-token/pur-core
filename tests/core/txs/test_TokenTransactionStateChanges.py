...
class TestTokenTransactionStateChanges(TestCase):
    ...
    def test_apply_token_txn(self):
        ...
        """
        initial_balances = [pur_pb2.AddressAmount(address=self.alice.address, amount=1000),
                            pur_pb2.AddressAmount(address=self.bob.address, amount=2000)]
    ...
    def test_apply_state_changes_owner_not_in_address_state(self):
        ...
        """
        initial_balances = [pur_pb2.AddressAmount(address=self.bob.address, amount=1000)]
        self.params["initial_balances"] = initial_balances
    ...
    def test_apply_token_txn_signed_by_slave_purss(self):
        """
        Alice creates a token, gives herself and Bob some tokens.
        But she uses a purSS slave to sign it.
        """
        initial_balances = [pur_pb2.AddressAmount(address=self.alice.address, amount=1000),
                            pur_pb2.AddressAmount(address=self.bob.address, amount=2000)]
        slave = get_slave_purss()
        self.params["initial_balances"] = initial_balances
        self.params["purss_pk"] = slave.pk
        self.params["master_addr"] = self.alice.address
        tx = TokenTransaction.create(**self.params)
        tx.sign(slave)

        # Now that we have the Slave purSS address, we should add it to AddressState so that apply_state_changes()
    ...
    def test_revert_token_txn_signed_by_slave_purss(self):
        """
        Alice creates a token, gives herself and Bob some tokens.
        But she uses a purSS slave to sign it.
        Can we undo it?
