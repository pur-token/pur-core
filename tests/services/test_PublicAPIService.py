...
from pypurlib.pypurlib import str2bin, hstr2bin, bin2hstr
...
from pur.core.TransactionMetadata import TransactionMetadata
...
from tests.misc.helper import get_alice_purss, get_bob_purss, replacement_getTime, set_pur_dir
...
class TestPublicAPI(TestCase):
    ...
    def test_getNodeState(self):
        ...
        purnode.set_chain_manager(chain_manager)
        purnode._p2pfactory = p2p_factory
        purnode._pow = p2p_factory.pow

        service = PublicAPIService(purnode)
        node_state = service.GetNodeState(request=pur_pb2.GetNodeStateReq, context=None)

        # self.assertEqual(__version__, node_state.info.version)  # FIpurE
        self.assertEqual(pur_pb2.NodeInfo.UNSYNCED, node_state.info.state)
        self.assertEqual(23, node_state.info.num_connections)
        # self.assertEqual("testnet", node_state.info.network_id)  # FIpurE
    ...
    def test_getKnownPeers(self):
        ...
        purnode._p2pfactory = p2p_factory
        purnode._pow = p2p_factory.pow
        purnode.peer_manager = Mock()
        purnode.peer_manager.known_peer_addresses = ['127.0.0.1', '192.168.1.1']

        service = PublicAPIService(purnode)
        response = service.GetKnownPeers(request=pur_pb2.GetKnownPeersReq, context=None)
    ...
    def test_getStats(self):
        ...
        service = PublicAPIService(purnode)
        request = pur_pb2.GetStatsReq()
        stats = service.GetStats(request=request, context=None)

        # self.assertEqual(__version__, stats.node_info.version)  # FIpurE

        self.assertEqual(pur_pb2.NodeInfo.UNSYNCED, stats.node_info.state)
        self.assertEqual(23, stats.node_info.num_connections)
        # self.assertEqual("testnet", stats.node_info.network_id)  # FIpurE
    ...
    def test_getStats_timeseries(self):
        with MockedBlockchain.create(10) as mock_blockchain:
            service = PublicAPIService(mock_blockchain.purnode)
            request = pur_pb2.GetStatsReq(include_timeseries=1)
            stats = service.GetStats(request=request, context=None)
    ...
    def test_getAddressState(self):
        with set_pur_dir('no_data'):
            ...
            optimized_address_state = OptimizedAddressState.create(address=alice_purss.address,
                                                                   nonce=25,
                                                                   balance=10,
            ...
            purnode.set_chain_manager(chain_manager)
            purnode._p2pfactory = p2p_factory
            purnode._peer_addresses = ['127.0.0.1', '192.168.1.1']
            ...
            context = Mock(spec=ServicerContext)
            request = pur_pb2.GetAddressStateReq()
            request.address = get_alice_purss().address
            response = service.GetAddressState(request=request, context=context)
            context.set_code.assert_not_called()

            self.assertEqual(alice_purss.address, response.state.address)
    ...
    def test_getObject(self):
        ...
        with set_pur_dir('no_data'):
            ...
            addr1_state = OptimizedAddressState.create(address=bob_purss.address,
                                                       nonce=25,
                                                       balance=10,
                                                       ots_bitfield_used_page=0,
                                                       transaction_hash_count=0,
            ...
            context = Mock(spec=ServicerContext)
            request = pur_pb2.GetObjectReq()
            request.query = bob_purss.address
            response = service.GetObject(request=request, context=context)
            context.set_code.assert_not_called()
            self.assertTrue(response.found)
            self.assertIsNotNone(response.address_state)

            self.assertEqual(bob_purss.address, response.address_state.address)
            ...
            tx1 = TransferTransaction.create(
                addrs_to=[bob_purss.address],
                amounts=[125],
                message_data=None,
                fee=19,
                purss_pk=bob_purss.pk,
                master_addr=alice_purss.address)
            ...
            self.assertEqual(alice_purss.address, response.transaction.tx.master_addr)
            self.assertEqual(bob_purss.pk, response.transaction.tx.public_key)
            self.assertEqual(tx1.txhash, response.transaction.tx.transaction_hash)
            self.assertEqual(b'', response.transaction.tx.signature)

            self.assertEqual(bob_purss.address, response.transaction.tx.transfer.addrs_to[0])
            ...
            block = Block.create(dev_config=config.dev,
                                 ...
                                 prev_timestamp=10,
                                 transactions=[],
                                 miner_address=alice_purss.address,
                                 seed_height=0,
                                 seed_hash=None)
            ...
            Block.put_block_number_mapping(db_state,
                                           block.block_number,
                                           pur_pb2.BlockNumberMapping(headerhash=block.headerhash),
                                           None)
            context = Mock(spec=ServicerContext)
            request = pur_pb2.GetObjectReq()
    ...
    def test_getLatestData(self):
        with set_pur_dir('no_data'):
            ...
            for i in range(0, 4):
                for j in range(1, 3):
                    txs.append(TransferTransaction.create(addrs_to=[bob_purss.address],
                                                          amounts=[i * 100 + j],
                                                          message_data=None,
                                                          fee=j,
            ...
            for j in range(10, 15):
                tx = TransferTransaction.create(addrs_to=[bob_purss.address],
                                                amounts=[1000 + j],
                                                message_data=None,
                                                fee=j,
                                                purss_pk=get_alice_purss().pk)
            ...
            purnode = purNode(mining_address=b'')
            purnode.set_chain_manager(chain_manager)
            purnode.get_block_from_index = MagicMock(return_value=None)
            ...
            request = pur_pb2.GetLatestDataReq(filter=pur_pb2.GetLatestDataReq.ALL,
                                               offset=0,
                                               quantity=3)
    ...
    def test_getAddressFromPK(self):
        ...
        request = pur_pb2.GetAddressFromPKReq(pk=bytes(pk))
    ...
    @patch('pur.core.config.DevConfig', new_callable=PropertyMock)
    def test_getMiniTransactionsByAddress(self, mock_dev_config):
        ...
        with set_pur_dir('no_data'):
            ...
            for i in range(0, 100):
                ...
                tx.sign(alice_purss)
                txs.append(tx)
            addresses_set = {bob_purss.address, alice_purss.address}
            block = Block.create(config.dev, 10, b'', 100000000, txs,
                                 alice_purss.address, seed_height=0, seed_hash=None)
            ...
            TransactionMetadata.update_tx_metadata(db_state, block, None)
            request = pur_pb2.GetMiniTransactionsByAddressReq(address=alice_purss.address,
                                                              item_per_page=10,
                                                              page_number=1)
    ...
    def test_getTransaction(self):
        ...
        context = Mock(spec=ServicerContext)
        request = pur_pb2.GetTransactionReq()
        ...
        self.assertEqual(alice_purss.address, response.tx.master_addr)
        self.assertEqual(bob_purss.pk, response.tx.public_key)
        self.assertEqual(tx1.txhash, response.tx.transaction_hash)
        self.assertEqual(b'', response.tx.signature)

        self.assertEqual(bob_purss.address, response.tx.transfer.addrs_to[0])
    ...
    def test_getTotalBalance(self):
        with set_pur_dir('no_data'):
            ...
            purss1 = get_alice_purss()
            purss2 = get_alice_purss(4)
            purss3 = get_bob_purss(4)
            address_state1 = OptimizedAddressState.create(address=purss1.address,
                                                          nonce=25,
                                                          balance=1000,
                                                          ots_bitfield_used_page=0,
                                                          transaction_hash_count=0,
                                                          ...
                                                          slaves_count=0,
                                                          multi_sig_address_count=0)
            address_state2 = OptimizedAddressState.create(address=purss2.address,
                                                          nonce=25,
                                                          balance=2000,
                                                          ots_bitfield_used_page=0,
                                                          transaction_hash_count=0,
                                                          ...
                                                          slaves_count=0,
                                                          multi_sig_address_count=0)
            address_state3 = OptimizedAddressState.create(address=purss3.address,
                                                          nonce=25,
                                                          balance=3000,
                                                          ots_bitfield_used_page=0,
                                                          transaction_hash_count=0,
            ...
            context = Mock(spec=ServicerContext)
            request = pur_pb2.GetTotalBalanceReq()
            request.addresses.extend([purss1.address, purss2.address, purss3.address])
    ...
    def test_getOTS(self):
        with set_pur_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = pur_pb2.GetOTSReq(address=alice_purss.address)
    ...
    def test_getBlock(self):
        with set_pur_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = pur_pb2.GetBlockReq(header_hash=b.headerhash)
    ...
    def test_getBlockByNumber(self):
        with set_pur_dir('no_data'):
            ...
            Block.put_block_number_mapping(db_state,
                                           b.block_number,
                                           pur_pb2.BlockNumberMapping(headerhash=b.headerhash),
                                           None)
            db_state.get_block_by_number = MagicMock(return_value=b)

            context = Mock(spec=ServicerContext)
            request = pur_pb2.GetBlockByNumberReq(block_number=b.block_number)
